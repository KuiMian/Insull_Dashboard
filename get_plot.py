...
def get_columnDataSource_and_dates(wind_data, solar_data):
    dates = np.array(wind_data['datetime'], dtype=np.datetime64)
    wind_data['datetime_str'] = wind_data['datetime'].apply(lambda x: as_str_time(x))
    wind_data['bottom'] = [0] * len(dates)

    source = ColumnDataSource(
        data=dict(date=dates, speed=wind_data['windspeed'], gust=wind_data['windgust'], bottom=wind_data['bottom'],
                  datetime_str=wind_data['datetime_str'], temp=solar_data['temp'], cloud=solar_data['cloudcover'],
                  radiation=solar_data['solarradiation'], energy=solar_data['solarenergy']))

    return source, dates


def get_plot(wind_data, solar_data, region):
    source, dates = get_columnDataSource_and_dates(wind_data, solar_data)
    ...
    p_wind = figure(title=region + '     windspeed / windgust(m/s)', width=2000, height=400,
    ...
    speed_plot = p_wind.line('date', 'speed', line_color='#fdae61', line_width=2, line_alpha=0.9,
                             ...
                             source=source)
    p_wind.circle('date', 'speed', line_color='green', line_alpha=0.4, fill_color="white", size=5,
                  legend_label='windspeed',
                  source=source)

    gust_plot = p_wind.line('date', 'gust', line_width=0.5, line_dash="4 4", color='firebrick', legend_label='windgust',
                            source=source)
    p_wind.circle('date', 'gust', line_color='red', fill_color="white", size=5, legend_label='windgust', source=source)
    ...
    p_solar = figure()
    p_solar = figure(title=region + '     solarradiation(W/m^2) / solarenergy(MJ/m^2)', width=2000, height=400,
                     tools=TOOLS,  # toolbar_location=None,
                     x_axis_type="datetime", x_axis_location="above",
    ...
    radiation_plot = p_solar.line('date', 'radiation', line_color='#fdae61', line_width=2, line_alpha=0.9,
                             ...
                             source=source)
    p_solar.circle('date', 'radiation', line_color='green', line_alpha=0.4, fill_color="white", size=5,
                  legend_label='solarradiation',
                  source=source)
    ...
    p_solar.circle('date', 'energy', line_color='red', fill_color="white", size=5, legend_label='solarenergy', source=source, y_range_name='energy')
    ...
    p_cloud_and_temp = figure(title=region + '     cloudcover(%) / temperature(°C)', width=2000, height=400,
                              tools=TOOLS,  # toolbar_location=None,
                              x_axis_type="datetime", x_axis_location="above",
    ...
    p_cloud_and_temp.line('date', 'temp', line_color='#F5E2A7', line_width=3, line_alpha=0.5, legend_label='temperature',
                          source=source)
    temp_plot = p_cloud_and_temp.circle('date', 'temp', line_color='#FDC072', fill_color='white', size=5,
    ...
    cloud_plot = p_cloud_and_temp.circle('date', 'cloud', line_color='#77AED1', fill_color='white', size=5,
    ...
    select = figure(title="Range tool", width=2000, height=130,
